...
/* ---------- 状态 ---------- */
let CORE_CFG = null;
let CORE_ITEMS = [];            // [{id,text,dim:'R'|'J'|'D', dir: +1|-1, domain?, w?}]
...
function startSurveyCore(){
  ...
  updateProgress(0); renderOneCore(0);
}
function updateProgress(done){
  const p = $('#progress'); if(p) p.textContent = `${done} / ${CORE_ITEMS.length}`;
}
...
function estimateTheta(ans){
  ...
  const beta = CORE_CFG?.core?.theta?.beta_RE ?? 0.6;
...
function classify(th){
  const P = (CORE_CFG?.core?.classify?.prototypes || []).map(p=>({
    macro:p.id, label:p.name, R:p.R, J:p.J, E:p.E
  }));
  const d2 = p => (th.R-p.R)**2 + (th.J-p.J)**2 + (th.E_p-p.E)**2;
  const scored = P.map(p=>({ ...p, d:Math.sqrt(d2(p)) }))
                  .sort((a,b)=>a.d-b.d)
                  .map((p,i)=>({ macro:p.macro, sim:+(1/(1+p.d)).toFixed(4), d:+p.d.toFixed(3) }));
